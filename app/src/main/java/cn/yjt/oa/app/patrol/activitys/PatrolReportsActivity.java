...
public class PatrolReportsActivity extends TitleFragmentActivity implements
    ...
    private final String REPORT_PATROL = "巡检明细";
    private final String REPORT_LEAK = "漏检明细";
    private final String REPORT_SUMMARY = "巡检汇总";
    ...
    protected void onCreate(Bundle savedState) {
        ...
        setContentView(R.layout.activity_patrol_reports);
        initQueue();
        initChoiceTimeView();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            /*记录操作 1706*/
                OperaEventUtils.recordOperation(OperaEvent.OPERA_DOWNLOAD_PATROL_REPORT);
            ...
            /*记录操作 1707*/
                OperaEventUtils.recordOperation(OperaEvent.OPERA_EMAIL_PATROL_REPORT);
    ...
    private void sendEmail(final String email) {
        ...
        PatrolApiHelper.emailPatrolExcel(sendEmailListener
                ,email
                ,getEmailModule()
                ,mBeginDate
                ,mEndDate);
//      MeetingApiHelper.emailSigninExcel(sendEmailListener, email, mMeetingId);
    ...
    private boolean checkReportType() {
        ...
        if (REPORT_CHECK.equals(reportType)) {
            mRequestUrl = PatrolApiHelper.MODULE_PATROL_CHECK;
        } else if (REPORT_PATROL.equals(reportType)) {
            mRequestUrl = PatrolApiHelper.MODULE_PATROL_DETAIL;
        } else if (REPORT_LEAK.equals(reportType)) {
            mRequestUrl = PatrolApiHelper.MODULE_PATROL_LEAK;
        } else if (REPORT_SUMMARY.equals(reportType)) {
            mRequestUrl = PatrolApiHelper.MODULE_PATROL_SUMMARY;
    ...
    private void download() {
        ...
        MobclickAgent.onEvent(this,
                "patrol_reports_download");
        try {
            String url = new AsyncRequest.Builder()
                    .setModule(getDownloadModule())
					.addDateQueryParameter(
							mBeginDate,
							mEndDate)
                    .build().getRequest().getUrl();

            fileRequest = new FileRequest(url, PatrolReportsActivity.this,
                    FileUtils.getUserFolder(), PatrolReportsActivity.this);
    ...
    public static void launch(Context context) {
        Intent intent = new Intent(context, PatrolReportsActivity.class);
    ...
    private String getEmailModule(){
        return mRequestUrl+PatrolApiHelper.PATROL_SENDMAIL;
    }

    /**获取下载报表的url*/
    private String getDownloadModule(){
        return mRequestUrl+PatrolApiHelper.PATROL_DOWNLOAD;
    }
