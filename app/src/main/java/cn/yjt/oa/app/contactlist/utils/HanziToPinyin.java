...
class HanziToPinyin {
    ...
    private HanziToPinyinToken getToken(char character) {
    ...
    public ArrayList<HanziToPinyinToken> get(final String input) {
        ...
        for (int i = 0; i < inputLength; i++) {
            ...
            if (character == ' ') {
                ...
                if (tokenType != HanziToPinyinToken.LATIN && sb.length() > 0) {
                    addToken(sb, tokens, tokenType);
                }
                tokenType = HanziToPinyinToken.LATIN;
                sb.append(character);
            } else {
                HanziToPinyinToken t = getToken(character);
                if (t.type == HanziToPinyinToken.PINYIN) {
                    if (sb.length() > 0) {
                        addToken(sb, tokens, tokenType);
                    }
                    tokens.add(t);
                    tokenType = HanziToPinyinToken.PINYIN;
                } else {
