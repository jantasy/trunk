...
public class CustomViewAbove extends ViewGroup {
	...
	private List<WeakReference<SlidingInterruptor>> mInterruptor = new ArrayList<WeakReference<SlidingInterruptor>>();
	...
	/**
	 * Simple implementation of the {@link com.jeremyfeinstein.slidingmenu.lib.CustomViewAbove.OnPageChangeListener} interface with
	 * stub implementations of each method. Extend this if you do not intend to
	 * override every method of {@link com.jeremyfeinstein.slidingmenu.lib.CustomViewAbove.OnPageChangeListener}.
	 */
	...
	public void addInterruptor(SlidingInterruptor interruptor) {
		if (interruptor != null && !mInterruptor.contains(interruptor)) {
			mInterruptor.add(new WeakReference<SlidingInterruptor>(interruptor));
		}
	}
	...
	private boolean allowTouchInterrupted(MotionEvent ev) {
		Iterator<WeakReference<SlidingInterruptor>> iterator = mInterruptor.iterator();
		while(iterator.hasNext()){
			WeakReference<SlidingInterruptor> next = iterator.next();
			if(next.get() == null){
				iterator.remove();
			}else{
				if (next.get().getUserVisibleHint(ev)) {
					if (next.get().allowTouchInterrupted(ev)) {
						return true;
					}
				}
			}
