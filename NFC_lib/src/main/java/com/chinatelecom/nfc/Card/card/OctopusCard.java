...
import com.chinatelecom.nfc.Card.tech.FeliCa;
...
final class OctopusCard {
	...
	static String load(NfcF tech, Resources res) {
		...
		for (byte i = 0; p < N; ++i) {
			...
			data[p++] = (Util_.toInt(r.getBlockData(), 0, 4) - 350) / 10.0f;
		}
	...
	private static String parseName(int system, Resources res) {
		if (system == SYS_OCTOPUS)
			return res.getString(R.string.nfc_name_octopuscard);

		if (system == SYS_SZT)
			return res.getString(R.string.nfc_name_szt_f);
	...
	private static String parseInfo(FeliCa.Tag tag, Resources res) {
		...
		final String p = res.getString(R.string.nfc_lab_pmm);
	...
	private static String parseBalance(float[] value, int count, Resources res) {
		...
		final String c = res.getString(R.string.nfc_lab_cur_hkd);

		r.append("<b>").append(s).append(" <font color=\"teal\">");

		for (int i = 0; i < count; ++i)
			r.append(Util_.toAmountString(value[i])).append(' ');
