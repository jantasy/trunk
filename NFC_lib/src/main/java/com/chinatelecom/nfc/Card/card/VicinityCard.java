...
package com.chinatelecom.nfc.Card.card;
...
final class VicinityCard {
	...
	static String load(NfcV tech, Resources res) {
		...
		try {
			...
			data = Util_.toHexString(rsp, 0, rsp.length);
	...
	private static String parseName(int type, Resources res) {
		if ((type & SYS_SZLIB) == SYS_SZLIB) {
			...
			if ((type & DEP_SZLIB_CENTER) == DEP_SZLIB_CENTER)
				dep = res.getString(R.string.nfc_name_szlib_center);
			else if ((type & DEP_SZLIB_NANSHAN) == DEP_SZLIB_NANSHAN)
				dep = res.getString(R.string.nfc_name_szlib_nanshan);
			else
				dep = null;

			final String srv;
			if ((type & SRV_BOOK) == SRV_BOOK)
				srv = res.getString(R.string.nfc_name_lib_booktag);
			else if ((type & SRV_USER) == SRV_USER)
				srv = res.getString(R.string.nfc_name_lib_readercard);
			else
				srv = null;
		...
		return res.getString(R.string.nfc_name_unknowntag);
	}

	private static String parseInfo(byte[] id, Resources res) {
		final StringBuilder r = new StringBuilder();
		final String i = res.getString(R.string.nfc_lab_id);
		r.append("<b>").append(i).append("</b> ")
				.append(Util_.toHexStringR(id, 0, id.length));
	...
	private static String parseSzlibData(int type, byte[] raw, byte[] sta,
		...
		if ((type & SRV_USER) == SRV_USER)
			sid = res.getString(R.string.nfc_lab_user_id);
		else
			sid = res.getString(R.string.nfc_lab_bktg_sn);
		...
		if ((type & SRV_BOOK) == SRV_BOOK) {
			...
			if ((type & DEP_SZLIB_NANSHAN) == DEP_SZLIB_NANSHAN) {
				if (cat == 0x10)
					scat = res.getString(R.string.nfc_name_bkcat_soc);
				else if (cat == 0x20) {
					if (raw[11] == (byte) 0x84)
						scat = res.getString(R.string.nfc_name_bkcat_ltr);
					else
						scat = res.getString(R.string.nfc_name_bkcat_sci);
				} else
					scat = null;
			} else {
				scat = null;
			...
			if (scat != null) {
				final String scl = res.getString(R.string.nfc_lab_bkcat);
