 ...
 * 
 */
public class MenuHorizontalScrollView extends ViewGroup {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				mLastMotionX = x;
				int tmp  = getScrollX();
				int tmp2 = findViewById(R.id.left_sliding_tab).getWidth();
				menuSlide(deltaX);
			}
			break;
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				if (menuOut == true)
					// 绠�澶村��宸�锛�浠ｈ〃���������
					this.menuBtn.setBackgroundResource(R.drawable.nfc_menu_fold);
				else
					this.menuBtn.setBackgroundResource(R.drawable.nfc_menu_unfold);
			}
			mTouchState = TOUCH_STATE_REST;
			break;
		case MotionEvent.ACTION_CANCEL:
	...
	public void clickMenuBtn() {
		if (!menuOut) {
			...
			final int availableToScroll = getChildAt(
					...
					- getScrollX() - getWidth();
			for(int i = 1; i <availableToScroll;i+=3){
				menuSlide(i);
	 ...
	 * 婊���ㄥ�鸿��锟�?
	 */
	private void menuSlide( int xPosition) {

		if (xPosition < 0) {
			if (getScrollX() > 0) {
				menuOut = true;
				 scrollBy(Math.max(-getScrollX(), xPosition), 0);
			}
		} else if (xPosition > 0) {
			final int availableToScroll = getChildAt(
					getChildCount() - 1).getRight()
			...
			if (availableToScroll > 0) {
				menuOut = false;
				 scrollBy(Math.min(availableToScroll, xPosition), 0);
			}
		}
		if (menuOut == true)
			// 绠�澶村��宸�锛�浠ｈ〃���������
